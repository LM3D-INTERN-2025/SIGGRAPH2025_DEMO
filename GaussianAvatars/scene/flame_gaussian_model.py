...
class FlameGaussianModel(GaussianModel):
    def __init__(self, sh_degree : int, coord, disable_flame_static_offset=False, not_finetune_flame_params=False, painted_tex_path="", n_shape=300, n_expr=100):
        ...
        # print("debug painted_tex_path:", painted_tex_path)
        if painted_tex_path == "":
    ...
    def training_setup(self, training_args):
        ...
        if training_args.train_texture:
            ...
            self.flame_model._tex_alpha.requires_grad = True
            param_text = {'params': [ self.flame_model._tex_painted, self.flame_model._tex_alpha ], 'lr': training_args.texture_lr, "name": "texture"}
            self.optimizer.add_param_group(param_text)
    ...
    def save_ply(self, path, opt):
        ...
        if opt.with_texture:
            ...
            tex_alpha_path = Path(path).parent / "flame_texture_alpha.png"
            flame_tex = self.flame_model._tex_painted.detach().cpu().numpy()[0,:,:,:].transpose(1, 2, 0)  # (H, W, C)
            flame_tex_alpha = self.flame_model._tex_alpha.detach().cpu().numpy()[0,:,:,:].transpose(1, 2, 0)  # (H, W, 1)
            flame_tex_alpha = np.tile(flame_tex_alpha,(1,1,3))
            print("debug tex alpha", flame_tex_alpha.shape)
            ####
            flame_tex = (flame_tex * 255).astype(np.uint8)
            flame_tex = Image.fromarray(flame_tex)
            flame_tex.save(str(tex_path))
            flame_tex_alpha = (flame_tex_alpha * 255).astype(np.uint8)
            flame_tex_alpha = Image.fromarray(flame_tex_alpha)
            flame_tex_alpha.save(str(tex_alpha_path))
